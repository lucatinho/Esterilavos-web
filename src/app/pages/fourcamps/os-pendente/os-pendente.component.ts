...
@Component({
  selector: 'app-os-pendente',
  templateUrl: './os-pendente.component.html',
  styleUrls: ['./os-pendente.component.css']
})
export class OsPendenteComponent implements OnInit {
  filter = {
    ano: '2021',
    mes: '06',
    tipo: 'Corretiva'
  ...
  enviar(dados){
    if (confirm("Você tem certeza que deseja enviar a OS nº " + dados.idOS)) { 
      ...
      //  troca de informações
        dados.envEmail = true
        let usuarioTemp = dados.usuario.login
        dados.usuario = {}
        dados.usuario.login = usuarioTemp
